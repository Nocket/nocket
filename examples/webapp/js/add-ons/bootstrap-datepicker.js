...
!function($) {
	...
	fill : function() {
		...
		if (prevMonth.getUTCDay() == this.weekStart) {
			...
			// ISO also states week starts on Monday, but we can be
			// more abstract here.
			var
			// Start of current week: based on weekstart/current
			// date
			ws = new Date(+prevMonth
				+ (this.weekStart - prevMonth.getUTCDay() - 7)
				% 7 * 864e5),
			// Thursday of this week
			th = new Date(+ws + (7 + 4 - ws.getUTCDay()) % 7
				* 864e5),
			// First Thursday of year, year from thursday
			yth = new Date(+(yth = UTCDate(th.getUTCFullYear(), 0,
				1))
				+ (7 + 4 - yth.getUTCDay()) % 7 * 864e5),
			// Calendar week: ms between thursdays, div ms per day,
			// div 7 days
	...
	click : function(e) {
		...
		case 'span':
			...
			if (target.is('.month')) {
				...
				this._setDate(UTCDate(year, month, day, 0, 0,
					0, 0));
			    }
			} else {
