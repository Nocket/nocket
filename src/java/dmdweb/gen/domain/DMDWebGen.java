...
import dmdweb.gen.domain.DMDWebGen.GenerationContext.FileData;
import dmdweb.gen.domain.ref.DomainClassReferenceFactory;
import dmdweb.gen.domain.ref.DomainReferenceFactoryI;
import dmdweb.gen.domain.visitor.DMDWebGenVisitor;
import dmdweb.gen.domain.visitor.DomainElementVisitorI;

public class DMDWebGen {

    final private static Logger log = LoggerFactory.getLogger(DMDWebGen.class);

    public static class GenerationProcessDMDWebGenContext<E extends AbstractDomainReference> extends
    DMDWebGenContext<E> {
	...
	public GenerationProcessDMDWebGenContext(Boolean generatePanel, String srcDir, String genDir,
		LayoutStrategy layoutStrategy, DomainReferenceFactoryI refFactory, GenerationContext generationContext) {
	...
	List<FileData> fileDataList = new ArrayList<DMDWebGen.GenerationContext.FileData>();
	...
	builder.append("jfd.retention.strategy=").append(new WebDomainProperties().getJFDRetentionStrategy())
	.append("\r\n");
    ...
    public void generateHTML(Class<?> domainClass, Boolean generatePanel, String sourceDir, String genDir,
	    LayoutStrategy layoutStrategy, GenerationContext generationContext) {
	...
	if (generationContext != null) {
	    // If there is a need for some statistic, an the specialized DMDWebGenContext will be used
	    context = new GenerationProcessDMDWebGenContext<DomainClassReference>(
		    generatePanel,
		    sourceDir, genDir, layoutStrategy, refFactory, generationContext);
	} else {
	    context = new DMDWebGenContext<DomainClassReference>(generatePanel, sourceDir, genDir, layoutStrategy,
		    refFactory);
	}
	...
	new DomainProcessor<DomainClassReference>(context, new MultiPassStrategy<DomainClassReference>() {
	    ...
	    @Override
	    public DomainElementVisitorI<DomainClassReference> createVisitor(
		    DMDWebGenContext<DomainClassReference> _dmdWebGenContext) {
		return new DMDWebGenVisitor<DomainClassReference>(_dmdWebGenContext);
	    }
	...
	System.err
	.println("Usage:\tjava "
		+ DMDWebGen.class.getName()
		+ " -s <SRC_DIR> -g <GEN_DIR> [-t (page | panel)] [-y (bootstrap | table)] [-l <CSS_FILE>]* <JAVA_CLASS_1> (<JAVA_CLASS_2> ... <JAVA_CLASS_n>)");
	System.err
	.println("Params:\t-s\tThe directory where HTML-Output will be written into package specific directories. Usually the \"src\" directory of the project.");
	System.err
	.println("\t-g\tThe directory where Constants-Class-Output will be written into package specific directories. Usually the \"gen\" directory of the project.");
	System.err
	.println("\t-t\tThe type of HTML artefact to create. By default, the generator tries to derive the type from existing Page/Panel classes. Pages are the fallback.");
	System.err.println("\t-y\tSpecifies the kind of mask layouting the generated HTML is prepared for.");
	System.err
	.println("\t...\tFQDN to java classes for which the html should be generated. The classes have to be in the classpath. Usually something like \"some.package.SomeType\".");
	System.err
	.println("Help:\tThis tool loads the given java classes and uses reflection to parse them in order to create html output for them.");
	System.err.println("\tThe output files are generated into the package folder of the given java class.");
	System.err
	.println("\tFiles that are written to SRC_DIR are merged with fields that have been added to the specific java class.");
