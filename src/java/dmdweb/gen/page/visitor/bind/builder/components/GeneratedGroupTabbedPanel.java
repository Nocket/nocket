...
import dmdweb.component.tabs.DMDTabbedPanel;
import dmdweb.gen.i18n.I18NLabelModel;
...
public class GeneratedGroupTabbedPanel extends DMDTabbedPanel<ITab> {
	...
	isLocalizationWicket = e.getContext().getConfiguration().isLocalizationWicket();
	...
	for (Map.Entry<String, Class<?>> entry : mapGroupNamePanelClass.entrySet()) {
	    getTabs().add(createTab(entry.getKey(), entry.getValue(), model));
	}
    }

    private void analyseMethod(Class<? extends Object> domainObjectClass, Map<String, Class<?>> mapGroupNamePanelClass,
	    Method method) {
	...
	if (!mapGroupNamePanelClass.containsKey(groupname)) {
	    mapGroupNamePanelClass.put(groupname, getPanelClass(domainObjectClass, groupname));
	}

    }

    private Class<?> getPanelClass(Class<? extends Object> domainObjectClass, String groupname) {

	String panelClassName = domainObjectClass.getName() + "_" + groupname.replace(".", "_") + "Panel";
    ...
    protected ITab createTab(String groupname, final Class<?> panelClass, final IModel<?> model) {
	...
	return new AbstractTab(Model.of(groupname)) {
		...
		try {
		    constructor = (Constructor<Panel>) panelClass.getConstructor(new Class<?>[] { String.class,
			    IModel.class });
	...
	final AjaxSubmitLink link = new AjaxSubmitLink(linkId) {
		...
		DMDWebGenPageContext context = new DMDWebGenPageContext(getParent());
		try {
		    webGenGuiServiceProvider.registerAjaxRequestTarget(context, target);
		    SynchronizerHelper.updateAllForms(context, target);
		} finally {
		    webGenGuiServiceProvider.unregisterAjaxRequestTarget(context, target);
		}
	...
	Method groupChoicer = ReflectionUtil.findGroupChoicer(domainObjectClass);
	...
	if (lastSelectedTabIndex != -1) {
	    oldTab = ((AbstractTab) getTabs().get(lastSelectedTabIndex)).getTitle().getObject();
	}
	String newTab = ((AbstractTab) getTabs().get(getSelectedTab())).getTitle().getObject();
	...
	try {
	    ...
	    String[] groupnames = (String[]) groupChoicer.invoke(defaultModelObject, (Object[]) null);
	    for (int i = 0; i < groupnames.length; i++) {
		Assert.notNull(mapGroupNamePanelClass.get(groupnames[i]),
			"Invalid group name in group choicer found: name = '" + groupnames[i] + "'");
		getTabs().add(createTab(groupnames[i], mapGroupNamePanelClass.get(groupnames[i]), defaultModel));
	    }
