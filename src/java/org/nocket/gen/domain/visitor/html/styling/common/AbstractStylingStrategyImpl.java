...
/**
 * Abstrakte Basisklasse f�r die Styling-Strategien. Implementierung ist der
 * alten Klasse
 * org.nocket.gen.domain.visitor.html.layout.AbstractHtmlLayoutStrategy
 * entnommen und wurde hier leicht angepasst.
 *
 * @author Thomas.Veit@Bertelsmann.de
 *
 */
public abstract class AbstractStylingStrategyImpl implements StylingStrategyI {
	...
	protected DMDWebGenContext<?> context;

	public AbstractStylingStrategyImpl() {
		this(null);
	}

	protected AbstractStylingStrategyImpl(DMDWebGenContext<?> context) {
	...
	protected WebDomainProperties getProperties() {
		if (context != null) {
			return context.getDomainProperties();
		}
		return new WebDomainProperties();
	}
	...
	protected boolean isInputButton(Element e) {
		return (e instanceof Input)
				&& ((Input) e).getAttribute(ATTR_TYPE) != null
				&& ((Input) e).getAttribute(ATTR_TYPE).equalsIgnoreCase(
						Input.button);
	}
	...
	public Element createChoicerProperty(ChoicerPropertyElement<?> e) {
		...
		if (ReflectionUtil.isMultivalued(e.getMethod()))
			select.addAttribute(SelectElement.SELECTIONTYPE_ATTRIBUTE,
					SelectElement.MULTISELECT_VALURE);
		select.setID(e.getWicketId());
		return wrapSelectComponent(e, select);
	}

	private Element wrapSelectComponent(ChoicerPropertyElement<?> e,
			Select select) {
	...
	protected Element wrapCheckboxComponent(CheckboxPropertyElement<?> e,
			Input input) {
		return wrapComponent(e, input);
	}
	...
	public Element createMultivalueProperty(MultivaluePropertyElement<?> e) {
		...
		if (getProperties().getHTMLTableExampleContent()) {
			// TODO JL: CSS-Klasse ist kopiert aus enericDataTablePanel.html.
			// Das müssen wir statt dessen rausparsen
			table.addAttribute(ATTR_CLASS,
	...
	protected THead createExampleTableHeader(MultivaluePropertyElement<?> e) {
		...
		TR tr = new TR();
		// TODO JL: CSS-Klasse stammt aus Analyse zur Applikationslaufzeit.
		// Lässt die sich auch irgendwo rausparsen?
		tr.addAttribute(ATTR_CLASS, "header");
		boolean isFirstColumn = true;
		for (MultivalueColumnElement dataColumn : e.getColumns()) {
			TH th = new TH();
			// TODO JL: Link und class darf nur rein, wenn man später auch
			// sortieren kann (List vs. Collection)
			th.addAttribute(ATTR_CLASS, isFirstColumn ? "wicket_orderUp"
					: "wicket_orderNone");
	...
	protected TBody createExampleTableBody(MultivaluePropertyElement<?> e) {
		...
		// Adding two lines shows the alternatiing color effect and gives a
		// better impression in general
		tbody.addElement(createExampleTableBodyLine(e, "even"));
		tbody.addElement(createExampleTableBodyLine(e, "odd"));
		return tbody;
	}

	private TR createExampleTableBodyLine(MultivaluePropertyElement<?> e,
			String rowClass) {
		TR tr = new TR();
		int numberOfCells = e.getColumns().size()
				+ e.getButtonElements().size();
		// Adding a non-breakable space in the first cell causes the line to
		// become a realistic height
	...
	protected Element wrapTableComponent(MultivaluePropertyElement<?> e,
			Table table) {
		return table;
	}
